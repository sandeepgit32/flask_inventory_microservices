<template>
  <div>
    <PageHeader title="Supply Transactions" subtitle="Track purchases from suppliers">
      <template #actions>
    ...
    <EmptyState 
      ...
      title="No supply transactions yet"
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="tx in transactions" :key="tx.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="table-cell">
                <div>
                  <p class="font-medium text-gray-900">{{ tx.supplier_name }}</p>
              ...
              <td class="table-cell">
                <span class="font-semibold text-emerald-600">${{ tx.total_cost?.toFixed(2) }}</span>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    >
      <form @submit.prevent="createTransaction" class="space-y-4">
        <div class="border-b border-gray-100 pb-4 mb-4">
          <h4 class="font-medium text-gray-900 mb-3">Supplier Information</h4>
...
import { supplyTransactionService } from '../services/api'
...
const createTransaction = async () => {
  try {
    ...
    await supplyTransactionService.create(payload)
